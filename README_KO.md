...
교체하고(Windows에서는 기존 파일을 옆으로 옮김), 새 릴리즈의 노트를 출력합니다.
시스템 디렉터리에 설치돼 있으면 관리자 권한으로 실행하세요. `GITHUB_TOKEN`을
설정하면 API 호출 한도가 올라갑니다.
...
버전 조회는 **백그라운드**에서 수행하며 포그라운드는 절대 기다리지 않으므로,
오프라인/air-gap 환경에서도 명령이 느려지지 않습니다 — 확인은 타임아웃 후 조용히
무시(soft-fail)됩니다. 안내는 캐시된 결과(하루 한 번 갱신)로 출력하며, **출력이
터미널일 때만** 표시하므로 스크립트나 파이프 출력을 방해하지 않습니다. 아무것도
자동 설치하지 않으며 업그레이드는 항상 명시적인 `hftools update`입니다. 현재 확인
상태(최신/업데이트 가능/GitHub 접속 불가)는 `hftools doctor`에서 볼 수 있습니다.
`HFTOOLS_NO_UPDATE_CHECK=1`로 전체 기능을 끌 수 있습니다.
